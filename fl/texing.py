...
def tasktexing():
    ...
    while(True):
        ...
        if sc.shot() and sc.image_check(c.img_sc_path,c.screen_size):                                                          ## 截图
            ...
            if sc.is_readysum():
                setState('readysum')
                if lastState == 'readysum' and lastTime > 3:
                    auto.toclick(419,759)
                continue
            if sc.is_resume():
                setState('resume')
            ...
            if sc.is_friend():
                ...
                continue
            if sc.is_battle():
                #auto.toclick(121,617)
                auto.toclick(205,670)
                time.sleep(0.3 + random.random()/10)
                #auto.toclick(302,679)
                auto.toclick(205,670)
            ...
            if sc.is_event():
                flow.toTag(1)
            okx, oky = sc.find_xy(c.ok_flag_img_path)
            cancelx, cancely = sc.find_xy(c.cancel_flag_img_path)
            if okx>0 and oky>0 and cancelx>0 and cancely>0:
                flow.toTag(1)
                continue
